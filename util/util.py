...
def save_network(net, label, epoch, opt):
    ...
    save_path = os.path.join(opt.checkpoints_dir, opt.store_name, save_filename)
...
def load_network(net, label, epoch, opt):
    save_filename = '%s_net_%s.pth' % (epoch, label)
    save_dir = os.path.join(opt.checkpoints_dir, opt.store_name)
    ...
    # if label == 'Corr':
    #     save_filename = '%s_net_%s.pth' % ('match', 'Corr')
    #     save_dir = os.path.join(opt.checkpoints_dir, opt.store_name)
    #     save_path = os.path.join(save_dir, save_filename)
    #     matching_weights = torch.load(save_path)
    #     matching_keys = matching_weights.keys()
    #
    #     reg_weights = OrderedDict()
    #     for key in matching_keys:
    #         if 'regression' in key:
    #             reg_weights[key] = matching_weights[key]
    #
    #     weights.update(reg_weights)

        # weights_load = OrderedDict()
        # for key in weights.keys():
        #     # new_key = key.replace('layer5', 'layer_for_corr')
        #     if 'regression' in key:
        #         continue
        #     weights_load[key] = weights[key]
        #     # weights.update(weights_load)
...
def print_current_errors(opt, epoch, i, errors, t):
    ...
    print(message)
    log_name = os.path.join(opt.checkpoints_dir, opt.store_name, 'loss_log.txt')
