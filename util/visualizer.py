...
class Visualizer():
    def __init__(self, opt):
        ...
        if self.use_html:
            self.web_dir = os.path.join(opt.checkpoints_dir, opt.store_name, 'web')
            self.img_dir = os.path.join(self.web_dir, 'images')
            print('create web directory %s...' % self.web_dir)
            util.mkdirs([self.web_dir, self.img_dir])
        self.log_name = os.path.join(opt.checkpoints_dir, opt.store_name, 'loss_log.txt')
        self.eval_log_name = os.path.join(opt.checkpoints_dir, opt.store_name, 'eval_log.txt')
    ...
    def plot_current_errors_b(self, iters, errors):
        if not hasattr(self, 'plot_data_b'):
            self.plot_data_b = {'X': [], 'Y': [], 'legend': list(errors.keys())}
        self.plot_data_b['X'].append(iters)
        self.plot_data_b['Y'].append([errors[k].detach().cpu().numpy().mean() for k in self.plot_data_b['legend']])
        self.vis.line(
            X=np.stack([np.array(self.plot_data_b['X'])] * len(self.plot_data_b['legend']), 1),
            Y=np.array(self.plot_data_b['Y']),
            opts={'title': self.name + ' loss over time B',
                  'legend': self.plot_data_b['legend'],
                  'xlabel': 'iterations',
                  'ylabel': 'loss'},
            win=self.display_id)
    ...
    def plot_current_errors(self, iters, errors):
        ...
        self.plot_data['Y'].append([errors[k].detach().cpu().numpy().mean() for k in self.plot_data['legend']])
        self.vis.line(
